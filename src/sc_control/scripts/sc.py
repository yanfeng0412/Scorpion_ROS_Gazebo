...
class sc:
    """Client ROS class for manipulating Scorpion in Gazebo"""
    def __init__(self,ns ='/sc/'):
        self.ns = ns 
        self.legs = None
        self.joints = None
        self.angles = None 
        self._sub_joints = rospy.Subscriber(ns + 'joint_states',JointState,self._cb_joints,queue_size =1)# Sub = subscriber
        ...
        self._pub_joints ={} #Creating Joint publisher and put data into an array
        for j in self.joints:
            for i in self.legs:
                p = rospy.Publisher(ns+'_'+j+'postion_controller/command', Float64, queue_size =1)
            self._pub_joints[j] = p
        rospy.sleep(1)

        self._pub_cmd_vel = rospy.Publisher(ns + 'cmd_vel', Twist, queue_size=1)
    
    def walk_vel(self,x,y,t):
        msg = Twist()
        msg.linear.x = x
        msg.linear.y = y
        msg.angular.z = t
    ...
    def _cb_joints(self,msg): ##call back function?? not sure 
